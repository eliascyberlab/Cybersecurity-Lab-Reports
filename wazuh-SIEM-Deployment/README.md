...
The goal of this project was to set up a central security monitoring system using a SIEM/XDR architecture (Security Information and Event Management / Extended Detection and Response). This system acts as a central "security brain," collecting security data and monitoring threats across several different computers and devices on a network.
...
This report shows exactly how the central security system was installed, secured, and connected to agent devices.
...
### 3.1 The OS Environment and Physical Infrastructure

The central security manager was set up on a dedicated Ubuntu Server machine. Rather than using virtualization, the operating system runs directly on the hardware to maximize processing efficiency. To ensure complete network isolation and security, the server is physically connected via an Ethernet cable to a secondary, separate router rather than the home's main internet router.

To maintain a secure and headless administration workflow, all deployment commands, configurations, and system initializations were executed remotely from a Debian Linux workstation via a secure SSH connection. This approach keeps the core server streamlined, leaving maximum system memory available for sorting data, indexing logs, and running the secure web dashboard.

### 3.2 Downloading the Installer

To begin, the official, all-in-one installation script was downloaded directly from the vendor's secure distribution network:
...
```
![fixed-ip](/wazuh-SIEM-Deployment/images/static-ip.png)
...
The installation was verified by confirming that the logs displayed `Connected to the server` and that the agent status successfully updated to `Agent is now online`.

### 7.2 Device 2: The Second Windows Laptop (win11-workstation-02)

To expand the deployment testing, the exact same process was mirrored to establish telemetry tracking for a physical laptop running Windows 11 Home. A new deployment string was generated via the setup wizard, and this asset was labeled `win11-workstation-02`. The tracking daemon package was then installed via an administrative terminal window, allowing the endpoint to successfully register with the system.
